 ...
 *
 * 상단: ☰(사이드바) + 벨(빨간 점 → 알림함 /notif-inbox-lab — 히스토리 필
 * 대체, 2026-08-04) + 찜 필. 구좌 헤더 = [타이틀 ›] 공통 규격(20 Semibold,
 * "더보기" 금지). 웹 셰브런은 광학 보정(30/600/translateY-2), 실기기는
 * SF chevron.right.
 *
 * curation-lab(공존안)을 복사해 개조한 파일이라 공존 전용 코드(대화 블록
 * 등)가 도달 불가 데드 상태로 남아 있음 — 실통합 시 삭제.
 *
...
export default function ExploreLabScreen() {
  ...
  // 핸드오프 — 공존안과 유일하게 다른 지점. 대화를 이 화면에 쌓지 않고
  // 쿼리를 들고 Chat 화면으로 전환한다 (뒤로 오면 스크롤 위치 보존).
    ...
    <View style={styles.root}>
      <Animated.ScrollView
        ...
        contentContainerStyle={[
          ...
          { paddingTop: topInset + TOP_PILL_HEIGHT + Spacing.two },
        ]}
